...
@Composable
fun DashboardBottomSheetDeleteArtist(
    ...
    val launcher =
        rememberLauncherForActivityResult(contract = ActivityResultContracts.StartIntentSenderForResult()) {
            if (it.resultCode == Activity.RESULT_OK) {
                deleteMusic(Selected(listOf(artist)))
                onClick()
            }
        }
    ...
    fun delete() {
        getListOfUrisViewModel(Selected(listOf(artist)))
    ...
    ListItem(
        ...
        icon = {
            Icon(
                Icons.Filled.Delete,
                contentDescription = stringResource(R.string.deleteArtist)
            )
        },
